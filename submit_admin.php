    ...
    if (!empty($_FILES['image_profile']['name'])) {
        ...
        if($check !== false) {
            if($imageFileType != "jpg" && $imageFileType != "png" && $imageFileType != "jpeg") {
                echo "<script>alert('ขออภัย, อนุญาตเฉพาะไฟล์ JPG, JPEG, PNG เท่านั้น'); window.location.href = 'adminadd.php';</script>";
            } else {
                if (move_uploaded_file($_FILES["image_profile"]["tmp_name"], $target_file)) {
                    ...
                    if ($stmt->execute()) {
                        echo "<script>alert('บันทึกข้อมูลคาเฟ่เรียบร้อยแล้ว'); window.location.href = 'admin.php';</script>";
                    } else {
        ...
        echo "<script>alert('กรุณาใส่รูปภาพ'); window.location.href = 'adminadd.php';</script>";
    }
}
?>
